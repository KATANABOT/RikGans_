...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imggoblin }, templateButtons: button, footer: `Rpg Game By TuruDek`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdevil }, templateButtons: button, footer: `Rpg Game By TuruDek`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgbehemoth }, templateButtons: button, footer: `Rpg Game By TuruDek`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdemon }, templateButtons: button, footer: `Rpg Game By TuruDek`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
kirbotz.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdemonking }, templateButtons: button, footer: `Rpg Game By TuruDek`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
var button = [
{"urlButton": {"displayText": "MyOwner🔥","url": `https://wa.me/6285892832640`}},
{"quickReplyButton": {"displayText": "Sewabot","id": 'sewabot'}},
{"quickReplyButton": {"displayText": "Donasi","id": 'donasi'}},
{"quickReplyButton": {"displayText": "Status","id": 'ping'}}
		]
...
kirbotz.sendList(m.chat, `Haii *${pushname}*`, `Silahkan Pilih Di Bawah`, "CLICK", "Fitur Nulis Dek ", fejfjej, { quoted: m })
break
case 'nuliskiri':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
kirbotz.sendList(m.chat, `Haii *${pushname}*`, `Silahkan Pilih Di Bawah`, "CLICK", "Topup Game Dek ", secmmknya, { quoted: efff })
break
case 'topup5':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
if (!args[0]) return replyNya(`Example : \n${prefix + command} 946716486`)
...
break
case 'credits':
const sokl = "6285892832640@s.whatsapp.net"
teksits = `「 *Thanks To* 」
♕ Akira @${sokl.split("@")[0]}`
...
kirbotz.sendList(m.chat, `Haii *${pushname}*`, `Silahkan Pilih Di Bawah`, "CLICK", "Rpg Game Dek ", sectionnya, { quoted: fakekirbotz})
break
case 'slime':
case 'killslime':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
const botzkir = "6285892832640@s.whatsapp.net"
...
mbc = `Halo Kak @${sender.split("@")[0]} 
Saya RIKI Saya Disini Ingin Membantu Kalian Membuat Sticker Download Video/Lagu Youtube, Tiktok Dll.`
let buttons = [
...
│ ${ya}${Yy+=1} < [EVAL2]${ya}
│ ${ya}${Yy+=1} => [ASYNC]${ya}
│ ${ya}${Yy+=1} ${prefix}bugpc 6288224893756|9${ya}
...
kirbotz.send5Loc(m.chat, `On Kak Silahkan Pencet Button Di Bawah\nJika Button Tidak Kelihatan Ketik ${prefix}menu`, `Turu Deck`, global.thumb, btn)
}
break
case 'userkirbotz':{
...
kirbotz.send5ButImg(i, junn, `Turu Deck`, buffer, butoon)
} else if (/video/.test(mime)) {
let junn = `*_BROADCAST VIDIO_*${text ? '\n\n' + text : ''}`
kirbotz.send5Vid(i, junn, `Turu Deck`, buffer, butoon)
...
"businessOwnerJid": "6285892832640@s.whatsapp.net",
}
...
QRIS ALL : https://link.dana.id/qr/2od05sb0

Note : MAU SEWA & PREM APA MAU LIAT DOANG KAK
...
const djej = "6285892832640@s.whatsapp.net"
